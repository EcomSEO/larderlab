      ...
      <div className="p-6 md:p-8 grid md:grid-cols-2 gap-8">
        <div>
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-sm text-charcoal/80 mb-1">
                Bodyweight
              </label>
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 rounded-md border border-ink/20 px-3 py-2 font-mono bg-white focus:outline-none focus:ring-2 focus:ring-copper/40"
                />
                <div className="flex rounded-md overflow-hidden border border-ink/20">
                  <button
                    type="button"
                    onClick={() => setUnit("kg")}
                    className={`px-3 text-sm font-mono ${unit === "kg" ? "bg-ink text-paper" : "bg-white text-charcoal"}`}
                  >
                    kg
                  </button>
                  <button
                    type="button"
                    onClick={() => setUnit("lb")}
                    className={`px-3 text-sm font-mono ${unit === "lb" ? "bg-ink text-paper" : "bg-white text-charcoal"}`}
                  >
                    lb
                  </button>
                </div>
            ...
            <div>
              <label className="block text-sm text-charcoal/80 mb-1">Activity</label>
              <select
                value={activity}
                onChange={(e) => setActivity(e.target.value as ActivityLevel)}
                className="w-full rounded-md border border-ink/20 px-3 py-2 bg-white focus:outline-none focus:ring-2 focus:ring-copper/40"
              >
            ...
            <div>
              <label className="block text-sm text-charcoal/80 mb-1">Goal</label>
              <select
                value={goal}
        ...
        <div>
          ...
          <div className="mt-6 pt-6 border-t border-ink/10">
            <p className="font-mono text-xs uppercase tracking-wider text-copper mb-2">
              Per-meal split
            </p>
          ...
          <tbody>
            {result.proteinRanges.map((range) => (
              <tr key={range.label} className="border-b border-ink/5">
                ...
                <td className="py-2 text-right text-copper font-semibold">
                  {range.proteinG}
                </td>
              </tr>
            ))}
...
function PerMeal({ n, protein, cal }: { n: number; protein: number; cal: number }) {
  return (
    <div className="p-3 bg-paper/50 border border-ink/10 rounded text-center">
      <div className="text-copper text-xs uppercase tracking-wider mb-1">{n} meals</div>
      <div className="text-charcoal/90">{protein}g prot</div>
