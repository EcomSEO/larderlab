  ...
  return (
    <header className="border-b border-ink/10 bg-cream/95 backdrop-blur sticky top-0 z-40">
      <div className="mx-auto max-w-6xl px-6 py-4 flex items-center justify-between">
        <Link href="/" className="font-serif text-2xl text-ink font-semibold">
          Larderlab
        </Link>
        ...
        <nav className="hidden md:flex items-center gap-8 text-sm">
          ...
          >
            ...
            {guidesOpen && (
              <div
                role="menu"
                className="absolute top-full left-0 mt-2 w-72 bg-cream border border-ink/10 rounded-md shadow-lg p-2"
              >
                {hubs.map((hub) => (
                  <Link
                    key={hub.slug}
                    ...
                    role="menuitem"
                    className="block px-3 py-2 hover:bg-copper/10 rounded text-charcoal"
                  >
                    {hub.name}
                  </Link>
                ))}
          ...
          <Link
            href="/newsletter"
            className="rounded-md bg-ink px-4 py-2 text-cream hover:bg-copper transition"
          >
